#include "util/heaptimer.h"

void HeapTimer::siftup_(size_t i) {
  size_t j = (i - 1) >> 1;
  while (j >= 0 and heap_[i] < heap_[j]) {
    SwapNode_(i, j);
    i = j;
    j = (i - 1) >> 1;
  }
...
void HeapTimer::doWork(int id) {
  ...
  size_t const i = ref_[id];
  TimerNode const node = heap_[i];
  node.cb();
  del_(i);
}
...
void HeapTimer::tick() {
  ...
  while (!heap_.empty()) {
    TimerNode const node = heap_.front();
