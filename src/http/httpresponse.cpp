...
void HttpResponse::AddContent_(Buffer& buff) {
  int const srcFd = open((srcDir_ + path_).data(), O_RDONLY);
  if (srcFd < 0) {
  ...
  int* mmRet = (int*)mmap(
      nullptr, mmFileStat_.st_size, PROT_READ, MAP_PRIVATE, srcFd, 0);
  if (*mmRet == -1) {
    ErrorContent(buff, "File NotFound!");
    return;
  }
...
std::string HttpResponse::GetFileType_() {
  /* 判断文件类型 */
  std::string::size_type const idx = path_.find_last_of('.');
  if (idx == std::string::npos) { return "text/plain"; }
  std::string const suffix = path_.substr(idx);
...
void HttpResponse::ErrorContent(
    Buffer& buff, const std::string& message) const {
  std::string body;
  std::string status;
