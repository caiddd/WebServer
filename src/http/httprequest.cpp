...
bool HttpRequest::parse(Buffer& buff) {
  ...
  while (buff.ReadableBytes() && state_ != FINISH) {
    ...
    std::string const line(buff.Peek(), lineEnd);
    switch (state_) {
      case REQUEST_LINE:
...
void HttpRequest::ParsePost_() {
  if (method_ == "POST"
    ...
    if (DEFAULT_HTML_TAG.count(path_)) {
      int const tag = DEFAULT_HTML_TAG.find(path_)->second;
      LOG_DEBUG("Tag:%d", tag);
      if (tag == 0 || tag == 1) {
        bool const isLogin = (tag == 1);
...
void HttpRequest::ParseFromUrlencoded_() {
  ...
  int num = 0;
  size_t const n = body_.size();
  int i = 0, j = 0;

  for (; i < n; i++) {
    char const ch = body_[i];
    switch (ch) {
      case '=':
...
bool HttpRequest::UserVerify(
  ...
  MYSQL* sql;
  SqlConnRAII const give_me_a_name(&sql, SqlConnPool::Instance());
  assert(sql);
  ...
  while (MYSQL_ROW row = mysql_fetch_row(res)) {
    ...
    std::string const password(row[1]);
    /* 注册行为 且 用户名未被使用*/
    if (isLogin) {
      if (pwd == password) {
        flag = true;
  ...
  if (!isLogin && flag) {
    ...
    bzero(order, 256);
    snprintf(
        order, 256, "INSERT INTO user(username, password) VALUES('%s','%s')",
