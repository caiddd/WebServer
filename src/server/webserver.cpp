...
WebServer::WebServer(
    ...
    int trigMode,        // ET模式
    int timeoutMS,       // 超时时限
    bool OptLinger,      // 设置优雅退出
    int sqlPort,         // 数据库端口号
    const char* sqlUser, // 数据库用户名
    const char* sqlPwd,  // 数据库密码
    const char* dbName,  // 数据库名
    int connPoolNum,     // 数据库连接池大小
    int threadNum,       // 线程池数量
    bool openLog,        // 日志开关
    int logLevel,        // 日志等级
  ...
  if (openLog) {
    ...
    if (isClose_) {
      ...
      LOG_INFO(
          "SqlConnPool num: %d, ThreadPool num: %d", connPoolNum, threadNum);
    }
  }
}
...
void WebServer::Start() {
  ...
  while (!isClose_) {
    ...
    int const eventCnt = epoller_->Wait(timeMS);
    for (int i = 0; i < eventCnt; i++) {
      /* 处理事件 */
      int const fd = epoller_->GetEventFd(i);
      uint32_t const events = epoller_->GetEvents(i);
      if (fd == listenFd_) {
        DealListen_();
...
void WebServer::SendError_(int fd, const char* info) {
  assert(fd > 0);
  int const ret = send(fd, info, strlen(info), 0);
...
void WebServer::DealListen_() {
  ...
  do {
    int const fd = accept(listenFd_, (struct sockaddr*)&addr, &len);
    if (fd <= 0) { return; }
...
bool WebServer::InitSocket_() {
  ...
  /* 只有最后一个套接字会正常接收数据。 */
  ret = setsockopt(
      listenFd_, SOL_SOCKET, SO_REUSEADDR, (const void*)&optval, sizeof(int));
  if (ret == -1) {
